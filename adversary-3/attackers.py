...
class MiaAttack:
    def __init__(self, victim_model, victim_train_loader, victim_test_loader,
                 ...
                 trans_victim_model_list=[], trans_shadow_model_list=[],
                 device="cuda", num_cls=10, lr=0.001, weight_decay=5e-4,
                 optimizer="adam", scheduler="", epochs=100, batch_size=128, 
                 dataset_name="mnist", model_name="mnist", exploit_layer_list=[]):
    ...
    def _prepare_si(self):
        self.attack_in_losses, _ = self.shadow_model.predict_target_loss(self.shadow_train_loader)
        self.attack_out_losses, _ = self.shadow_model.predict_target_loss(self.shadow_test_loader)

        self.victim_in_losses, _ = self.victim_model.predict_target_loss(self.victim_train_loader)
        self.victim_out_losses, _ = self.victim_model.predict_target_loss(self.victim_test_loader)

        trans_attack_in_losses_list, trans_attack_out_losses_list = [], []
        trans_victim_in_losses_list, trans_victim_out_losses_list = [], []
        for trans_shadow_model, trans_victim_model in zip(self.trans_shadow_model_list, self.trans_victim_model_list):
            trans_attack_in_losses, _ = trans_shadow_model.predict_target_loss(self.shadow_train_loader)
            trans_attack_out_losses, _ = trans_shadow_model.predict_target_loss(self.shadow_test_loader)

            trans_victim_in_losses, _ = trans_victim_model.predict_target_loss(self.victim_train_loader)
            trans_victim_out_losses, _ = trans_victim_model.predict_target_loss(self.victim_test_loader)
        ...
        self.trans_attack_in_losses = torch.cat(trans_attack_in_losses_list, dim=1)
        self.trans_attack_out_losses = torch.cat(trans_attack_out_losses_list, dim=1)
        self.trans_victim_in_losses = torch.cat(trans_victim_in_losses_list, dim=1)
        self.trans_victim_out_losses = torch.cat(trans_victim_out_losses_list, dim=1)
    ...
    def kt_attack(self, mia_type="kt_loss", model_name="mia_fc"):
        ...
        trans_attack_losses = torch.cat([self.trans_attack_in_losses, self.trans_attack_out_losses], dim=0)
        ...
        trans_victim_losses = torch.cat([self.trans_victim_in_losses, self.trans_victim_out_losses], dim=0)
        ...
        save_folder = f"results/{self.dataset_name}_{self.model_name}/{self.exploit_layer_list[-1]}"
        if not os.path.exists(save_folder):
            raise FileNotFoundError("no pretrained transfer victim/shadow models")

        if mia_type == "kt_loss":
            new_attack_data = torch.cat([attack_losses, trans_attack_losses], dim=1)
            new_victim_data = torch.cat([victim_losses, trans_victim_losses], dim=1)

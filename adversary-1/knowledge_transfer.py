...
parser.add_argument('--total_layer_list', default=[], type=list)
...
def main(args):
    ...
    trivial_model_save_folder = base_folder + "/trivial_model"
    if not os.path.exists(f"{trivial_model_save_folder}/best.pth"):
        raise FileNotFoundError("no pretrained trivial model")
    trivial_model = BaseModel(args.model_name, num_cls=args.num_cls, input_dim=args.input_dim, device=device)
    trivial_model.load(f"{trivial_model_save_folder}/best.pth")

    if args.transfer_layer_list == [] or args.total_layer_list == []:
        raise ValueError("transfer_layer_list or total_layer_list is empty")
    remain_layer_list = [item for item in args.total_layer_list if item not in set(args.transfer_layer_list)]

    tranfer_mode = args.mode.split(',')

    # Train knowledge transfer victim model
    print(f"Train {args.mode} Victim Model")

    if "transfer" in tranfer_mode:
        transfer_victim_model_save_folder = f"{base_folder}/{args.transfer_layer_list[0]}/transfer_victim_model"
        ...
        print(f"Remain List: {remain_layer_list}")
        for n, m in trivial_model.model.named_modules():
            if n in remain_layer_list:
                transfer_m = copy.deepcopy(m)
                set_module(trans_victim_model.model, n, transfer_m)
    ...
    # Train knowledge transfer shadow model
    print(f"Train {args.mode} Shadow Model")
    if "transfer" in tranfer_mode:
        transfer_shadow_model_save_folder = f"{base_folder}/{args.transfer_layer_list[0]}/transfer_shadow_model"
